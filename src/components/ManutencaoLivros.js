...
const ManutencaoLivros = () => {
    ...
    const filtrarLista = async (campos) => {
        try {
            ...
            lista.data.lenght ? setLivros(lista.data) : alert("Não há livros com a palavra-chave pesquisada...");
        } catch (error) {
            alert(`Erro... Não foi possível obter os dados: ${error}`);
        }
    };
        ...
        <div className="container">
            <div className="row">
                ...
                <div className="col-sm-7">
                    <h4 className="fst-italic mt-3">Manutencao</h4>
                </div>
            ...
            <table className="table table-striped mt-3">
                ...
                <tbody>
                    {livros.map((livro, index) => (
                        <ItemLista key={livro.id} id={livro.id} titulo={livro.titulo} autor={livro.autor} ano={livro.ano} preco={livro.preco} foto={livro.foto} excluirClick={() => excluir(livro.id, livro.titulo)}
